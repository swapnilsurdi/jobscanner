// Job feed UI v2. Reads meta.json for the actual scan-run timestamp.
// Overwrite docs/app.js with this file's contents.
...
function fmtUpdated() {
  if (META && META.last_scan_at) return new Date(META.last_scan_at).toLocaleString();
...
function render() {
  ...
  els.count.textContent = rows.length === JOBS.length ? `${JOBS.length}` : `${rows.length} of ${JOBS.length}`;
}
...
els.clear.addEventListener('click', () => {
  state.query = ''; els.search.value = ''; activeCompanies.clear();
...
Promise.all([
  fetch('jobs.json', { cache: 'no-store' }).then(r => r.json()).catch(() => []),
  fetch('meta.json', { cache: 'no-store' }).then(r => r.json()).catch(() => null),
]).then(([jobs, meta]) => {
  ...
  els.empty.textContent = 'Failed to load data — ' + err.message;
});
